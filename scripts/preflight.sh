...
relint
( export CARGO_TERM_COLOR="always" RUSTFLAGS="-Dwarnings"; cargo clippy --no-default-features --all-targets -- -W clippy::pedantic -D warnings )

step "ci.yml / msrv: Check (all features)"
( export CARGO_TERM_COLOR="always" RUSTFLAGS="-Dwarnings"; cargo +1.88 check --lib --all-features )

step "ci.yml / feature-powerset: Powerset ({websocket, mqtt, rest} depth 2)"
( export CARGO_TERM_COLOR="always" RUSTFLAGS="-Dwarnings"; cargo hack check --lib --feature-powerset --depth 2 )

step "ci.yml / fmt: Check formatting"
( export CARGO_TERM_COLOR="always" RUSTFLAGS="-Dwarnings"; cargo fmt -- --check )

step "ci.yml / doc: Doc"
( export CARGO_TERM_COLOR="always" RUSTFLAGS="-Dwarnings" RUSTDOCFLAGS="-Dwarnings"; cargo doc --no-deps --all-features )
...
step "ci.yml / test: Test (default = websocket + mqtt + rest)"
( export CARGO_TERM_COLOR="always" RUSTFLAGS="-Dwarnings"; cargo test )

step "ci.yml / test: Test (no default features)"
( export CARGO_TERM_COLOR="always" RUSTFLAGS="-Dwarnings"; cargo test --no-default-features )
